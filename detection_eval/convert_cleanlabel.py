...
'''
Level of occlusion of each pedestrian i n the i mage. We consider
four l evels Fully_visible (no occlusion), Mostly_visible (more than 50%
visible), Severly_occluded (more than 50% occluded), Fully_occluded (no
visible at all).
'''
occlusion_dict = {  
    'Fully_visible': 0,
...
def move_frame(input_dir, output_dir, calib, seq_name, file_name, labels_2d,
    ...
    for label_id in label_ids:
        ...
        truncated = truncated_dict[label_2d['attributes']['truncated']]    
        occlusion = occlusion_dict[label_2d['attributes']['occlusion']]
        num_points = label_3d['attributes']['num_points']

        '''
        if num_points < 10 or occlusion == 3:
            continue
        '''
        if num_points < 10:
            continue
        ...
        label_lines.append(
            f"Pedestrian {truncated} {occlusion} {label_3d['observation_angle']} "
