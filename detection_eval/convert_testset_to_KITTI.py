...
def get_file_list(input_dir):
    ...
    def _label2filelist(path, key='labels'):
        seq_dicts = []
        print(os.path.join(input_dir, path))
    ...
    filelist = set.intersection(
            imgs, lower_ptcs, upper_ptcs)

    print(len(imgs), len(lower_ptcs),len(upper_ptcs), len(detections_2d),len(filelist))
    return {f: (detections_2d[f],)
...
def convert_jr2kitti(input_dir, output_dir, file_list):
    ...
    pool.starmap(
        move_frame,
        [(input_dir, output_dir, calib, seq_name, file_name, None, None,
          detection_2d, idx)
